...
def gen_ticket(email):
    email_hash = hashlib.sha256(email.encode()).hexdigest()[:7]
...
@app.route("/get_private_dishes", methods = ["POST"])
@jwt_required()
def get_private_dishes():
    user_info = get_jwt_identity()
    All_dis = db.private_dish_book.find({'email': user_info})
    ...
    for dish in dishes:
        ...
        private_dishes.append(dish_data)
    return jsonify(private_dishes), 200
